...
function getSectionDividerHTML(type) {
    ...
    if (!content) return '';
    return `<div class="sec-divider-wrap" style="grid-column: 3 / 6; text-align: center; margin: 16px auto 6px auto; display:flex; justify-content:center; align-items:center; ${dimStyle}">${content}</div>`;
}
...
function renderBackShaar(container, themeId) {
    ...
    const author = document.getElementById('inp-book-author')?.value || "";
    const year = document.getElementById('inp-book-year')?.value || "";
    const backText = document.getElementById('inp-book-back-text')?.value || "";
    ...
    <div class="theme-bounded-content" data-shaar-type="back" data-shaar-id="${shaarBoxId}" style="${boxStyle}">
      ...
      <div>
        ...
        <div style="font-size: 10.5pt; line-height: 1.6; max-width: 90%; margin: 10px auto; color: #333; white-space: pre-wrap;">
          ${backText}
        </div>
      </div>

...
async function loadExternalJSONAssets() {
    const assetsToLoad = [
        { key: 'shaar_main', file: 'shaar_main.json', type: 'shaar_main' },
        { key: 'shaar_sub', file: 'shaar_sub.json', type: 'shaar_sub' },
        { key: 'shaar_back', file: 'shaar_back.json', type: 'shaar_back' },
        { key: 'header_ornaments', file: 'header_ornaments.json', type: 'header' },
        { key: 'section_dividers', file: 'section_dividers.json', type: 'divider' },
        { key: 'note_rules', file: 'note_rules.json', type: 'note_rule' },
        { key: 'ornaments_left_right', file: 'ornaments_left_right.json', type: 'ornaments_lr' },
        { key: 'ornaments_up_down', file: 'ornaments_up_down.json', type: 'ornaments_ud' }
    ];
    ...
    for (const asset of assetsToLoad) {
        try {
            let items = null;
            if (window.TORANI_ASSETS && Array.isArray(window.TORANI_ASSETS[asset.key])) {
                items = window.TORANI_ASSETS[asset.key];
            } else {
...
async function loadExternalFonts() {
    try {
        let fonts = null;
        if (window.TORANI_ASSETS && Array.isArray(window.TORANI_ASSETS.fonts)) {
            fonts = window.TORANI_ASSETS.fonts;
        } else {
            const res = await fetch('fonts/fonts.json');
            if (res.ok) fonts = await res.json();
        }
...
/* ==========================================================================
   גשר Microsoft Word Add-in (שאיבת טקסט ישירה עם סגנונות כותרת)
   ========================================================================== */
async function importFromCurrentWordDoc() {
    if (typeof Word === 'undefined') {
        alert('פונקציה זו פועלת רק בתוך Microsoft Word כתוסף Taskpane.');
        return;
    }
    try {
        await Word.run(async (context) => {
            ...
            for (let i = 0; i < paragraphs.items.length; i++) {
                ...
                const builtin = (p.styleBuiltIn || '').toLowerCase();

                if (style.includes('heading 1') || style.includes('כותרת 1') || builtin.includes('heading1')) {
                    lines.push('כותרת 1 - ' + txt);
                } else if (style.includes('heading 2') || style.includes('כותרת 2') || builtin.includes('heading2')) {
                    lines.push('כותרת 2 - ' + txt);
                } else if (style.includes('heading 3') || style.includes('כותרת 3') || builtin.includes('heading3')) {
                    lines.push('כותרת 3 - ' + txt);
                } else if (style.includes('heading 4') || style.includes('כותרת 4') || builtin.includes('heading4')) {
                    lines.push('כותרת 4 - ' + txt);
                } else if (style.includes('heading 5') || style.includes('כותרת 5') || builtin.includes('heading5')) {
                    lines.push('כותרת 5 - ' + txt);
                } else if (style.includes('heading 6') || style.includes('כותרת 6') || builtin.includes('heading6')) {
                    lines.push('כותרת 6 - ' + txt);
                } else if (style.includes('heading 7') || style.includes('כותרת 7') || builtin.includes('heading7')) {
                    lines.push('כותרת 7 - ' + txt);
                } else {
                    lines.push(txt);
                }
            }

            const ed = document.getElementById('raw-input');
            if (ed) {
                ed.value = lines.join('\n\n');
                typesetDocument();
            }
        ...
        alert('שגיאה בשאיבת תוכן מ-Word: ' + err.message);
    }
}
...
if (typeof Office !== 'undefined') {
    Office.onReady((info) => {
        if (info.host === Office.HostType.Word) {
            const btn = document.getElementById('btn-word-import');
